...
const rightMenu = () => {
    ...
    const closeOther = () => store.dispatch('permission/removeOtherTagNav',currentRightTags) //removeOtherTagNav(currentRightTags)
...
const tagScroll = () => {
    const store = useStore()
    const { tagsList, cachedViews } = store.state.permission.tags
...
export default defineComponent({
    ...
    setup() {
        const store = useStore()
        const route = useRoute()
        const removeTag = (v) =>store.dispatch('permission/removeTagNav',{ cPath: route.path, tagsList: v })
        //const removeTag = (v) =>removeTagNav({ cPath: route.path, tagsList: v })

        const removeAllTagNav = () => store.dispatch('permission/removeAllTagNav')

        onMounted(() => {
            //addCachedViews({ name: route.name as string, noCache: route.meta.noCache as boolean })
            console.log({ name: route.name, noCache: false })
            store.dispatch('permission/addCachedViews',{ name: route.name, noCache: false })
        })
